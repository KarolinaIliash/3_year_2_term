    ...
    <table border="1" cellpadding="5" cellspacing="1" >
       <c:forEach items="${requestScope.userList}" var="userInfo" >
   		  <tr>
          	 ...
          	 <th style="color:red;">${Bundle.getObject("addBlacklist")}</th>
       	  </tr>
          <tr>
             ...
             <c:if test="${userInfo.user.inBlackList == false}">
             	...
             	<td><a href="setBlackList?phone=${userInfo.user.phone}&add=true">Add to black list</td>
             </c:if>
             <c:if test="${userInfo.user.inBlackList == true}">
             	<td>${Bundle.getObject("yes")}</td>
             	<td>${Bundle.getObject("alreadyThere")}</td>
             </c:if>
          </tr>
       </c:forEach>
    </table>
